...
function checkPasswordStrength(password) {
    const birth = localStorage.getItem("birth").split("-");
    const day = birth[1] + birth[2];
    const full_b = birth[0] + birth[1] + birth[2]
    if (birth!='Empty' && (password.indexOf(day)!=-1 || password.indexOf(full_b)!=-1)) {
        return 'Birthday Password😢 We recommend you change your password!';
    }
...
function RegisterUser() {
    ...
    var data = "idname=" + localStorage.getItem("name") + "&username=" + encodeURIComponent(username) + "&password=" + encodeURIComponent(password);
